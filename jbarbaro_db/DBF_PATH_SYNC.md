...
- `ALT_SKU` -> `barcode`
- `Itemmrc.dbf` + `Itemmqty.dbf` -> size/color variants and exact legacy POS barcodes with `--matrix-variants`
...
- Matrix variant mode uses each `Itemmqty.dbf` cell quantity instead of putting the total on one Shopify variant.
...
## Archive Shopify Products Missing From Item.dbf

Add `--archive-missing` to compare Shopify against every non-empty SKU in `Item.dbf`. The read-only
run fetches Shopify's catalog and writes `missing-products.csv` and `missing-products.json` in the
new full-sync report folder; it does not archive anything:
...
After reviewing `missing-products.csv`, add `--upload --yes` to upload the inventory and archive the
approved missing-SKU set:
...
The archive step runs only after all product upload batches complete without failures. Only products
marked with the `pos.sku` metafield by a previous rich POS sync are eligible; unrelated or manually
created Shopify products are left unchanged. Products with no Shopify SKU are also left unchanged. A
Shopify product with multiple variants is kept when any variant SKU exists in `Item.dbf`; products
whose variant list could not be read completely are left unchanged. `--stocked-only` does not affect
the comparison: zero-stock SKUs that still exist in `Item.dbf` count as present and are not archived
by this step.

## Preserve Existing POS Matrix Barcodes

Use `--matrix-variants` when old POS labels must scan in Shopify POS. Matrix products are built as
one Shopify product with separate size/color variants. `Itemmrc.dbf` supplies the row and column
labels, and `Itemmqty.dbf` supplies each cell's inventory.

The legacy barcode format is preserved exactly without the Code 39 start/stop asterisks:

```text
Base SKU: 21741
Cell: 1 1
Size: 30
Shopify variant SKU: 21741. 1 1
Shopify variant barcode: 21741. 1 1
Quantity: 1
```

Preview one matrix product:
...
Preview the complete catalog before uploading:
...
The full preview writes `matrix-variant-audit.csv` in its report folder. Review that file before the
live run. The sync service must be deployed with matrix variant support before using the live command.

Live upload after reviewing the preview and testing one SKU:
